Here is the list of commands that can be used to quickly install HAOBO ACH Edge using docker compose and connect to the server.
...
#### Step 1. Create the HAOBO ACH Edge Docker Compose file

HAOBO ACH Edge supports both **in-memory** and **Kafka** queues for message storage and communication between HAOBO ACH services.
It also supports **SQL** and **hybrid** database configurations.
In this guide, we’ll use the **in-memory** queue and an **SQL** database.
For more details about the hybrid setup, please refer to the official installation instructions on the <a href="https://thingsboard.io/docs/user-guide/install/edge/docker/#step-2-choose-queue-andor-database-services" target="_blank">HAOBO ACH documentation site</a>.

Now, create a Docker Compose file for the HAOBO ACH Edge service:
...
#### Step 2. Start HAOBO ACH Edge
Navigate to the directory containing the `docker-compose.yml` file and run the following command to start the HAOBO ACH Edge service:
